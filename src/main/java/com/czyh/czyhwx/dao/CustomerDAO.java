...
public interface CustomerDAO extends JpaRepository<TCustomer, String>, JpaSpecificationExecutor<TCustomer> {
	...
	@Query("select t from TCustomer t where t.fweixinId = ?1 and t.fstatus < 999 and t.ftype=1")
	...
	@Query("select t.id from TCustomer t where t.ftype = ?2 and t.fstatus = ?3 and t.fweixinId = ?1 ")
	TCustomer getOneByOpenid(String id, int i, int j);
