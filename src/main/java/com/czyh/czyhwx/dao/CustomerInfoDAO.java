...
public interface CustomerInfoDAO extends JpaRepository<TCustomerInfo, String>, JpaSpecificationExecutor<TCustomerInfo> {
	...
	@Modifying
	@Query("update TCustomerInfo t set t.fkykFlag = ?2 , t.finvalidTime = ?3  where t.fcustomerId = ?1")
	void updateKykFlag(String customerId, int fkykFlag, Date fupdateTime);
